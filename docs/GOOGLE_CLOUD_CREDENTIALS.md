...
The current Play Store release pipeline (see `PLAY_STORE_RELEASE_RUNBOOK.md`) uses Fastlane with a separate Play Store service account (`google-play-service-account.json` or `GOOGLE_PLAY_JSON_KEY_FILE`).
...
- [Play Store Release Runbook](PLAY_STORE_RELEASE_RUNBOOK.md) - Play Store deployment process
